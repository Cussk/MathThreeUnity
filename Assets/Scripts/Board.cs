...
public class Board : MonoBehaviour
{
    ...
    [Range(0, 1)]
    [SerializeField] private float chanceForCollectibles = 0.1f;
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (m_allGamePieces[i, j] == null && m_allTiles[i, j].tileType != TileType.Obstacle)
                {
                    ...
                    //if at top of game board and collectible available fill with a collectible
                    if ((j == height - 1) && CanAddCollectible())
                    {
                        piece = FillRandomCollectibleAt(i, j, falseYOffset, moveTime);
                        collectibleCount++;
                    }
                    //else fill with normal gamepieces
                    else
                    {
    ...
    GamePiece FillRandomGamePieceAt(int xCoordinate, int yCoordinate, int falseYOffset = 0, float moveTime = 0.1f)
    {
        if (IsWithinBounds(xCoordinate, yCoordinate))
        {
    ...
    GamePiece FillRandomCollectibleAt(int xCoordinate, int yCoordinate, int falseYOffset = 0, float moveTime = 0.1f)
    {
        if (IsWithinBounds(xCoordinate, yCoordinate))
        {
    ...
    List<GamePiece> FindMatches(int startX, int startY, Vector2 searchDirection, int minimumLength = 3)
    {
        ...
        for (int i = 1; i < maxValue - 1; i++)
        {
            ...
            {
                //if pieces color matches and not alredy in list and match value is not none, add to list
    ...
    IEnumerator ClearAndCollapseRoutine(List<GamePiece> gamePieces)
    {
        ...
        while (!isFinished)
        {
            ...
            //list of collectibles on bottom row
            List<GamePiece> collectedPieces = FindCollectiblesAtRow(0, true);

            List<GamePiece> allCollectibles = FindAllCollectibles();
            //gets all collectibles that are effected by bombs
            List<GamePiece> blockers = gamePieces.Intersect(allCollectibles).ToList();
            //add blockers to list to be decremented
            collectedPieces = collectedPieces.Union(blockers).ToList();

            collectibleCount -= collectedPieces.Count;

            //combine collectibles to master list to be cleared
            ...
            collectedPieces = FindCollectiblesAtRow(0, true);
            //add collectibles to matches list
        ...
        foreach (GamePiece piece in gamePieces)
        {
            if (piece != null)
            {
                ...
                if (bomb != null)
                {
                    ...
                    //remove collectibles that cannot be destroyed by bombs
    ...
    List<GamePiece> FindCollectiblesAtRow(int row, bool clearedAtBottomOnly = false)
    {
    ...
    List<GamePiece> FindAllCollectibles()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            List<GamePiece> collectibleRow = FindCollectiblesAtRow(i);
    ...
    {
        return (Random.Range(0.0f, 1.0f) <= chanceForCollectibles && collectiblePrefabs.Length > 0 && collectibleCount < maxCollectibles);
    }
    ...
    List<GamePiece> RemoveCollectibles(List<GamePiece> bombedPieces)
    {
        ...
        //exclude designated collectibles from list
